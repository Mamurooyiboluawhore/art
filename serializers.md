...
Create a `serializers.py` file within the post app
```bash
touch serializers.py
```
Open and edit the `serializers.py` file to create a `BlogPostSerializer` class
...
### USING SERIALIZERS IN DRF
To use serializers in DRF, create 
...
### Definition and Significance of Serializers
Serializers are classes that convert complex data types into native Python datatypes that can then be easily rendered
into JSON, XML or other formats. Serializers are used to convert data from model instances into native Python datatypes that can then be easily rendered into JSON, XML or other formats.
